...
public class HrController {
	...
	public String info(int id, Model model) {
		//선택한 사원정보를 DB에서 조회 -> 정보화면에 출력할 수 있도록 Model 객체에 담기
	...
	public String delete(int id) {
		//해당 사원정보를 DB에서 삭제하기 -> 목록화면으로 연결하기
	...
	public String modify(Model model , int id) {
		// 해당 사원정보를 DB에서 조회 -> 수정화면에 출력할 수 있도록 Model 객체에 담기
		model.addAttribute("vo",service.employee_info(id));
		
		// 부서,업무를 변경할 수 있도록 부서목록,업무목록을 조회 후 화면에 출력할 수 있도록 Model 객체에 담기
	...
	public String register(Model model) {
		// 부서와 업무와 매니저를 선택할 수 있도록 DB에서 조회 후 화면에 출력-> Model 객체에 담기
		model.addAttribute("departments", service.hr_department_list());
		model.addAttribute("jobs", service.hr_job_list());
